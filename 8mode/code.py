...
# ==============================
parser = argparse.ArgumentParser(description='Higher-Order Rectified Flow for Eight-Mode Dataset')
...
# ==============================
# Dataset Creation (Eight-Mode) - CHANGED FROM 4 TO 8
# ==============================
VAR = 0.3  # variance
M = args.M  # plotting range
COMP = 8  # number of Gaussian components (octagon vertices) - CHANGED FROM 4 TO 8

# Source distribution (π₀): octagon at smaller radius
...
# Target distribution (π₁): rotated octagon at larger radius
...
class RectifiedFlowHigherOrder():
    ...
    def get_train_tuple(self, z0, z1):
        ...
        else:  # exponential interpolation (default for eight-mode)
...
def draw_plot(rectified_flow, z0, z1, N=None):
    ...
    plt.title(f'Eight-Mode: {args.loss_config}\nAvg Dist: {avg_dist:.4f}', fontsize=14)
    ...
    if args.save_figs:
        ...
        filename = f"eightmode_{args.loss_config}_{args.interpolation}_{args.flow_type}{suffix}.png"
...
def main():
    ...
    if args.save_figs:
        suffix = "_curv" if args.christoffel else ""
        loss_path = f"eightmode_{args.loss_config}_{args.interpolation}_{args.flow_type}{suffix}_loss.png"
        os.makedirs(args.fig_dir, exist_ok=True)
